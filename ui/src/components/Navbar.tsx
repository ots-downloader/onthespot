...
const NAV_DETAILS: Record<
  WorkspaceTab | ManageTab,
  {
    label: string;
...
> = {
  ...
  queue: {
    ...
    labelKey: "download_queue",
    icon: Download,
  },
  accounts: { label: "Accounts", labelKey: "accounts", icon: Users },
  diagnostics: {
    label: "Diagnostics",
    labelKey: "diagnostics",
    icon: Activity,
  },
  logs: { label: "Server logs", labelKey: "server_logs", icon: Terminal },
...
const readNavOrder = <T extends string>(key: string, fallback: T[]): T[] => {
  try {
    ...
    if (
      ...
      stored.every((item) => fallback.includes(item))
    )
      return stored as T[];
  } catch {
  ...
  return (
    ...
    >
      ...
      <span className="relative shrink-0">
        <Icon
          className={`h-5 w-5 transition-transform group-hover:scale-105 ${active ? "text-[#1ed760]" : ""}`}
        />
        {indicator && (
...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  const [workspaceOrder, setWorkspaceOrder] = useState<WorkspaceTab[]>(() =>
    readNavOrder("ots-workspace-nav-order", WORKSPACE_TABS),
  );
  const [manageOrder, setManageOrder] = useState<ManageItem[]>(() =>
    readNavOrder("ots-manage-nav-order", MANAGE_TABS),
  );
  const [editingNavigation, setEditingNavigation] = useState(false);
  const [draggedNavigation, setDraggedNavigation] = useState<{
  ...
  const reorderNavigation = (group: "workspace" | "manage", target: string) => {
    if (
      !draggedNavigation ||
      draggedNavigation.group !== group ||
      draggedNavigation.id === target
    )
      return;
    const apply = <T extends string>(
      ...
      const next = [...current];
      const from = next.indexOf(draggedNavigation.id as T);
      const to = next.indexOf(target as T);
      if (from < 0 || to < 0) return;
      next.splice(from, 1);
      next.splice(to, 0, draggedNavigation.id as T);
      setCurrent(next);
      window.localStorage.setItem(key, JSON.stringify(next));
    };
    if (group === "workspace")
      apply(workspaceOrder, setWorkspaceOrder, "ots-workspace-nav-order");
    else apply(manageOrder, setManageOrder, "ots-manage-nav-order");
  ...
  return (
    <header className="z-40 flex w-full shrink-0 flex-col border-b border-[#282828] bg-[#121212] md:fixed md:inset-y-0 md:left-0 md:w-64 md:border-b-0 md:border-r">
      <div className="flex items-center justify-between px-5 py-4 md:block md:px-4 md:py-5">
        ...
        >
          ...
          <span>
            ...
            <span className="block text-[10px] font-semibold uppercase tracking-[0.18em] text-[#8f8f8f]">
              your music utility
            </span>
          </span>
        </button>
      ...
      <nav className="spotify-scrollbar flex items-center gap-1 overflow-x-auto px-3 pb-3 md:block md:space-y-1 md:overflow-visible md:px-3">
        <p className="hidden px-4 pb-2 pt-1 text-[10px] font-bold uppercase tracking-[0.2em] text-[#6f6f6f] md:block">
          {translate(language, "workspace", "Workspace")}
        </p>
        {workspaceOrder.map((id) => {
          const item = localizeNavItem(id);
          return (
            <TabButton
              key={id}
              id={id}
              {...item}
              active={activeTab === id}
              onActivate={() => onTabChange(id)}
              count={id === "queue" ? queueCount : undefined}
              indicator={id === "queue" && activeDownloads > 0}
              editing={editingNavigation}
              onDragStart={() =>
                setDraggedNavigation({ group: "workspace", id })
              }
              onDragOver={(event) => {
                if (editingNavigation) event.preventDefault();
              }}
              onDrop={(event) => {
                event.preventDefault();
                reorderNavigation("workspace", id);
              }}
              onDragEnd={() => setDraggedNavigation(null)}
            />
          );
        })}
        <p className="hidden px-4 pb-2 pt-5 text-[10px] font-bold uppercase tracking-[0.2em] text-[#6f6f6f] md:block">
          {translate(language, "manage", "Manage")}
        </p>
        {manageOrder.map((id) =>
          id === "notifications" ? (
            <button
              ...
              onDragStart={() => setDraggedNavigation({ group: "manage", id })}
              onDragOver={(event) => {
                if (editingNavigation) event.preventDefault();
              }}
              onDrop={(event) => {
                event.preventDefault();
                reorderNavigation("manage", id);
              }}
              onDragEnd={() => setDraggedNavigation(null)}
              onClick={onOpenNotificationHistory}
              className={`ots-nav-item group relative flex w-full items-center gap-4 rounded-lg px-4 py-3 text-left text-sm font-semibold text-[#b3b3b3] transition-all duration-200 hover:bg-[#1f1f1f] hover:text-white focus:outline-none ${editingNavigation ? "cursor-grab" : ""}`}
            >
              {editingNavigation && (
                <GripVertical
                  className="h-4 w-4 shrink-0 text-[#777]"
              ...
              <Bell className="h-5 w-5 text-[#b3b3b3] transition-transform group-hover:scale-105" />
              <span>
                {translate(
                  language,
            ...
            (() => {
              ...
              return (
                <TabButton
                  ...
                  active={activeTab === id}
                  onActivate={() => onTabChange(id)}
                  count={id === "accounts" ? accountCount : undefined}
                  editing={editingNavigation}
                  onDragStart={() =>
                    setDraggedNavigation({ group: "manage", id })
                  }
                  onDragOver={(event) => {
                    if (editingNavigation) event.preventDefault();
                  }}
      ...
      <div className="mt-auto hidden p-4 md:block">
        ...
        >
          ...
          <span>
            {editingNavigation
              ? translate(language, "done_editing", "Done editing")
              : translate(language, "edit_navigation", "Edit navigation")}
          </span>
        </button>
        <button
          onClick={toggleTheme}
        ...
        >
          ...
          <span>
            {isDarkMode === "dark"
              ? translate(language, "dark_mode", "Dark mode")
              : translate(language, "light_mode", "Light mode")}
          </span>
        </button>
        <p className="px-4 pt-3 text-center text-[10px] font-medium text-[#6f6f6f]">
          OnTheSpot {appVersion}
        </p>
      </div>
    </header>
  );
