        ...
        account_pool.append(
            {
                ...
                "account_type": "N/A",
                "bitrate": "N/A",
                "login": {"token": None, "refresh_token": None, "token_expiry": None},
            }
        )
        return False
...
def crunchyroll_get_episode_metadata(token, item_id):
    ...
    episode_data = make_call(
        f"{BASE_URL}/content/v2/cms/objects/{item_id.split('/')[0]}?ratings=true&images=true&locale=en-US",
        headers=headers,
    )
